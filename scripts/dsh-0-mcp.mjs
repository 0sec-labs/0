...
export function usage() {
  ...
  node scripts/dsh-0-mcp.mjs \\
    ...
    [--mcp-env <NAME> ...] [--entrypoint <0.js>] \\
    [--dsh-bin <dsh>] [--dry-run] \\
    "<one headless task>"

Starts DeepSeek Harness as an external headless host and mounts only the
0 stdio MCP tools. The runner creates a private temporary DSH patch and
removes it after dsh exits; it does not modify DSH profiles or 0 config.

--scope is required even though 0 mcp-server itself permits omission.
Use --mcp-env only for an explicit 0 child-process environment variable,
...
export function buildProfilePatch({ entrypoint, serverCwd, mcpArgs, mcpEnv }) {
  ...
  return `# Generated by scripts/dsh-0-mcp.mjs. Do not persist credentials here.
# This final overlay keeps the DSH tool registry but exposes only 0 MCP tools.
- id: tools
  config:
    mode: native
...
export async function runRunner(argv = process.argv.slice(2), env = process.env) {
  ...
  const entrypoint = options.entrypoint ?? join(repoRoot, "dist", "0.js");
  ...
  if (!existsSync(entrypoint)) {
    throw new UsageError(`0 entrypoint does not exist: ${entrypoint}; run pnpm build or pass --entrypoint`);
  }

  const tempDir = await mkdtemp(join(tmpdir(), "0-dsh-mcp-"));
  const patchPath = join(tempDir, "0-mcp.patch.yml");
  try {
...
if (isMain) {
  runRunner().then((exitCode) => {
    ...
    process.stderr.write(`dsh-0-mcp: ${error instanceof Error ? error.message : String(error)}\n\n${usage()}`);
