...
for (const finding of sortedFindings) {
  const ruleId = finding.templateId || finding.id || "0sec-finding";
  if (!rulesById.has(ruleId)) {
    rulesById.set(ruleId, {
      id: ruleId,
      name: finding.title || ruleId,
      shortDescription: {
        text: finding.title || "0sec finding",
      },
      fullDescription: {
        text: finding.description || "0sec detected a potential security issue.",
      },
  ...
  runs: [
    {
      ...
      results: sortedFindings.map((finding) => ({
        ruleId: finding.templateId || finding.id || "0sec-finding",
        level: severityToLevel(finding.severity),
        message: {
          text: [finding.description, finding.evidence?.analysis].filter(Boolean).join("\n\n") || finding.title || "0sec finding",
        },
        locations: [
          {
            physicalLocation: {
...
const gateMessage = severityThreshold === "none"
  ...
  : `0sec found ${qualifyingFindings.length} finding(s) at or above ${severityThreshold}; threshold allows ${countThreshold}.`;
...
lines.push("---");
lines.push("*Generated by [0sec](https://github.com/0sec-labs/0sec)*");
