...
export function keyHintsLength(pairs: readonly KeyHint[], sep: string): number {
  ...
  pairs.forEach((p, i) => {
    if (i > 0) n += sep.length;
    // `[glyph] label` — two bracket cells, a space, plus the parts.
    n += 2 + keyGlyph(p.key).length + 1 + p.label.length;
  });
  return n;
}
 ...
 * MUTED, so `[⇧⇥] mode · [⌃P] palette` reads as chords, not prose. Each
...
export function KeyHints({
  ...
  pairs.forEach((p, i) => {
    ...
    nodes.push(<text key={`key-${i}`} flexShrink={0} fg={TEXT}>{`[${keyGlyph(p.key)}]`}</text>);
