...
function buildIssueBody(finding: Finding): string {
  ...
  lines.push("---");
  lines.push(`*Exported by [0](https://github.com/0sec-labs/0) | Finding ID: \`${finding.id}\`*`);

  return lines.join("\n");
}

function getLabels(finding: Finding): string[] {
  const labels: string[] = ["0", `severity:${finding.severity}`, `category:${finding.category}`];
  return labels;
}
...
/**
 * Fetch all open issues with the "0" label to check for duplicates.
...
async function fetchExistingIssueTitles(repo: string, token: string): Promise<Set<string>> {
  ...
  while (true) {
    const url = `https://api.github.com/repos/${repo}/issues?state=open&labels=0&per_page=${perPage}&page=${page}`;
 ...
 * existing open issues that carry the "0" label.
 */
