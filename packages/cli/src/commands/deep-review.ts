 ...
 * `ZERO_DEEP_REVIEW_MAX_CANDIDATES` (and/or `--models` / `--attempts`) when a
...
function defaultMaxCandidates(): number {
  const raw = process.env["ZERO_DEEP_REVIEW_MAX_CANDIDATES"];
 ...
 *  `ZERO_DEEP_REVIEW_ATTEMPTS` for a deliberate deeper (best-of-N) run. */
const DEFAULT_ATTEMPTS = 1;
/** Env override for {@link DEFAULT_ATTEMPTS}; the `--attempts` flag still wins. */
function defaultAttempts(): number {
  const raw = process.env["ZERO_DEEP_REVIEW_ATTEMPTS"];
 ...
 * comma-separated `ZERO_DEEP_REVIEW_MODELS` (or pass `--models`) for a
 * deliberate multi-model run. The `--models` flag still overrides the env.
 */
function defaultModels(): string[] | undefined {
  const raw = process.env["ZERO_DEEP_REVIEW_MODELS"];
 ...
 *  without a real source tree. Match the `@0/core` signatures. */
...
export interface RunDeepReviewOptions {
   ...
   *  single provider-default model (or $ZERO_DEEP_REVIEW_MODELS). */
  models?: string[];
  /** Finder attempts per (candidate × lens × model). Default 1 (or
   *  $ZERO_DEEP_REVIEW_ATTEMPTS) — best-of-N is opt-in. */
  attemptsPerCandidate?: number;
  /** Max finders in flight. Default 8 (matches runHuntScan). */
  concurrency?: number;
  /** Max candidate files hunted (largest-first). Default 8 (or $ZERO_DEEP_REVIEW_MAX_CANDIDATES). */
  maxCandidates?: number;
  ...
  timeoutMs?: number;
  /** Hard scan-wide USD ceiling. Overrides $ZERO_COST_CEILING_USD. */
...
export async function runDeepReview(
  ...
  const hasEnvMaxOverride = !!process.env["ZERO_DEEP_REVIEW_MAX_CANDIDATES"];
  ...
  try {
    ...
    // Priority: --max-candidates flag > ZERO_DEEP_REVIEW_MAX_CANDIDATES env > auto-scale.
    ...
    let allocateAcrossLanes: typeof import("@0/core").allocateCandidatesAcrossLanes | undefined;
    if (opts.useThreatModel) {
      const threatModule = await import("@0/core");
...
async function deepReviewAction(target: string, opts: DeepReviewOpts): Promise<void> {
  ...
  const ceilingSource = opts.costCeiling ?? process.env["ZERO_COST_CEILING_USD"];
  ...
  const outcome = await runDeepReview({
    ...
    // flag is set; otherwise let runDeepReview resolve $ZERO_DEEP_REVIEW_MODELS.
...
export function registerDeepReviewCommand(program: Command): void {
  program
    ...
    .option("--models <a,b>", "Comma-separated finder models for diversity (default: single provider model, or $ZERO_DEEP_REVIEW_MODELS)")
    .option("--attempts <N>", `Finder attempts per candidate×lens×model, best-of-N (default ${DEFAULT_ATTEMPTS}, or $ZERO_DEEP_REVIEW_ATTEMPTS)`)
    .option("--concurrency <N>", `Max finders in flight (default ${DEFAULT_CONCURRENCY})`)
    .option("--cost-ceiling <usd>", "Hard scan-wide USD ceiling. Overrides $ZERO_COST_CEILING_USD.")
    .option("--max-candidates <N>", `Cap candidate files hunted, largest-first (default ${DEFAULT_MAX_CANDIDATES}, or $ZERO_DEEP_REVIEW_MAX_CANDIDATES)`)
