// `0 guide` — the agent-readable product and capability guide.
//
// Other clients and harnesses discover everything 0 offers from this one
...
const ONBOARDING = {
  ...
  steps: [
    "0 auth login",
    "0 project enroll <repository> --json",
    "0 project setup <repository> --json",
    "Review the proposal and per-run credit limit; save with project save. Saving alone leaves recurring checks paused.",
    "Optional: propose a few codebase review checks with 0 checks propose; list drafts and enable only those the developer approves.",
...
const CAPABILITIES: Capability[] = [
  {
    ...
    command: "0 project enroll https://github.com/org/repo --json",
    layer: "service",
    requiresAuth: true,
  ...
  {
    id: "review-checks",
    summary: "Save short review prompts for one codebase. Each approved check runs during its configured PR review.",
    when: "A developer wants Zero to check a behavior or code pattern on every relevant repository change.",
    command: "0 checks propose --name \"Tenant boundaries\" --prompt \"A user must never read another organization's records\"",
    layer: "service",
    requiresAuth: true,
    limitations: "Proposals are saved but not enabled. The coding agent should inspect the repository, propose a few checks, and ask the developer which to enable. Checks share the existing funded PR review; a suggested change is not a verified patch or an automatic repair PR. Requires a service release with review-check support.",
    next: ["Use 0 checks list to show saved drafts.", "After developer approval, enable a check with 0 checks enable <id> --yes."],
  },
  {
    id: "hosted-inference",
  ...
  },
  {
    ...
    command: "0 secure <repo> --test-command \"npm test\"",
    layer: "engine",
  ...
  },
  {
    ...
    command: "0 connect https://github.com/org/repo --setup-only",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 service start --repo https://github.com/org/repo --test-command \"npm test\"",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 service status <scan-id>",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 service wait <scan-id>",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 service cancel <scan-id>",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 service disconnect https://github.com/org/repo",
    layer: "service",
    requiresAuth: true,
  ...
  },
  {
    ...
    command: "0 review ./my-app --diff-base origin/main --changed-only",
    layer: "engine",
  },
  {
    id: "scan",
    ...
    command: "0 scan --target https://staging.example.com --scope ./scope.json",
    layer: "engine",
  },
  {
    id: "verify",
    ...
    command: "0 verify --finding ./finding.json",
    layer: "engine",
  },
  {
    id: "fix",
    summary: "Generate and validate a scoped single-file fix for one already-reproduced finding.",
    when: "You have a reproduced finding and a regression command. Prefer 0 secure for the full loop.",
    command: "0 fix ./my-app --finding ./finding.json --test-command \"npm test\"",
    layer: "engine",
  },
  {
    id: "learning",
  ...
  },
  {
    ...
    command: "0 secure <repo> --test-command \"npm test\" --rules \"minimal diffs; no new dependencies\"",
    layer: "engine",
  },
];
...
const ARCHITECTURE = {
  ...
  boundaries: {
    auth: "Cloud actions need `0 auth login` and the relevant service entitlement. Local/BYOK runs need no cloud account; provider requests can still use the network. A health response does not verify account identity or product access.",
    ...
    service: {
      ...
      note: serviceProbe === "ok"
        ...
        : serviceProbe === "unauthenticated"
          ? "The health probe rejected the credentials. Run 0 auth login; product access still needs its own checks."
...
function commandMetadata(command: Command) {
  return {
    name: commandPath(command).replace(/^0 /, ""),
...
function printHuman(topic: string | undefined, service: GuideServiceStates, commands: Command[]): void {
  ...
  if (!topic) {
    ...
    out.push("");
    out.push("Topics: 0 guide <capability-id> | 0 guide commands | 0 guide \"auth login\" | 0 guide architecture | 0 guide limits | 0 guide --json");
    ...
    out.push("Use 0 guide \"<command path>\" for its registered arguments and options.");
  } else {
    const cap = CAPABILITIES.find((c) => c.id === topic);
    const command = commands.find((c) => commandPath(c).replace(/^0 /, "") === topic);
    if (cap) {
      ...
      if (cap.requiresAuth) out.push("Requires: 0 auth login and the relevant service access");
  ...
  program
    ...
    .action(async (topic: string | undefined, opts: { format: string; json: boolean }) => {
      ...
      if (topic && !capability && !command && !["commands", "architecture", "limits"].includes(topic)) {
        throw new InvalidArgumentError(`Unknown topic '${topic}'. Run 0 guide for the list.`);
      }
