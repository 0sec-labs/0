/**
 * `0 review --profile` / `--target` validator tests. We register the command on
...
describe("0 review --profile validator", () => {
  beforeEach(() => {
  ...
  it("threads --emit pr + --base + --dry-run into runUnified (0#377)", async () => {
    await runCli([
      "review",
      "./somerepo",
      "--emit",
  ...
  it("loads valid --prior-findings JSON and forwards it unchanged", async () => {
    const dir = mkdtempSync(join(tmpdir(), "0-prior-findings-"));
