...
export async function runAssumptionHuntCli(sourceRoot: string, opts: AssumptionHuntOpts): Promise<{ exitCode: number; result: unknown }> {
  const { runAssumptionHunt, makeSkepticVerifier } = await import("@0/core");
  ...
  const modelPath = opts.modelPath
    ...
    : resolve(root, `.0/assumption-models/${subsystem.replaceAll("/", "_")}.json`);
...
export function registerAssumptionHuntCommand(program: Command): void {
  program
    ...
    .option("--model-path <path>", "Where the durable assumption model JSON lives (default under <source-root>/.0)")
    ...
    .option("--dynamic-witness", "v3: route dual-view candidates to the KASAN synthesize→boot→witness oracle (bypasses the static skeptic). Needs a KASAN VM env (ZERO_KERNEL_QEMU_*).")
