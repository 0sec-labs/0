...
export async function generatePdfReport(
  ...
  return new Promise<void>((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        Title: `0 Pentest Report - ${report.target}`,
        Author: "0",
...
function renderCoverPage(doc: PDFDoc, report: ScanReport): void {
  ...
  doc
    ...
    .text("0", PAGE_MARGIN, 200, { align: "center" });
...
function renderMethodology(doc: PDFDoc, report: ScanReport): void {
  ...
  doc
    ...
    .fillColor(COLORS.text)
    .text(
      "This report was generated by 0, a fully autonomous agentic pentesting framework. " +
...
function renderFooters(doc: PDFDoc): void {
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    doc
      ...
      .fillColor(COLORS.textLight)
      .text(
        `0 Pentest Report  |  Page ${i + 1} of ${pageCount}  |  CONFIDENTIAL`,
        PAGE_MARGIN,
        760,
