// Recon mode — surface enumeration for a domain/org (0#769, #924).
//
...
export interface ReconOptions {
  ...
  /**
   * Active subdomain brute-force config (0#924). OFF by default: omit it
...
async function parseSpecBody(body: string): Promise<ApiSpecSummary | undefined> {
  ...
  const dir = mkdtempSync(join(tmpdir(), "0-recon-"));
...
/**
 * Passive subdomain enumeration (0#769, wired to `./subdomains.ts`).
 *
...
export async function runRecon(domain: string, options: ReconOptions = {}): Promise<ReconResult> {
  ...
  // Active subdomain brute-force (0#924). OFF unless the caller opted in
