...
function setup(): { target: LinuxKernelTarget; artifactRoot: string } {
  const root = mkdtempSync(join(tmpdir(), "0-linux-adapter-"));
  roots.push(root);
