...
export class LinuxBootMatrixImportAdapter implements TargetResearchAdapter<LinuxBootMatrixTarget, LinuxBootMatrixCandidate, never, never> {
  ...
  async verify(_target: LinuxBootMatrixTarget, input: { candidates: LinuxBootMatrixCandidate[] }, ctx: ResearchContext): Promise<ResearchStageResult<ResearchFinding>> {
    ...
    for (const candidate of input.candidates) {
      ...
      if (passed) items.push({ finding: candidate.payload.finding, candidateId: candidate.id, grade: "reproduced", executionContext: { privilege: "unknown", basis: "declared" }, evidence: [{ stage: "verify", status: "passed", summary: "0 validated and hashed externally executed logs; 0 did not execute these boots", data: verdict, artifacts }] });
    }
