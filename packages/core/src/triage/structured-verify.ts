/**
 * Structured multi-step verification pipeline for 0 findings.
 *
...
function mapCategory(finding: Finding): VulnCategory | undefined {
  const map: Record<string, VulnCategory> = {
    "sql-injection": "sqli",
    xss: "xss",
    "code-injection": "ssti", // SSTI is a subclass of code injection in 0's taxonomy
...
- For blind SSRF: register an OAST handle with the \`oast_register\` tool, inject its http_url/dns_host as the SSRF target, then \`oast_poll\` — a token-matched DNS/HTTP callback is concrete proof (0#659). Fall back to an external collaborator (Burp, webhook.site) only if OAST is not deployed.
