...
describe("widening gadget rendering", () => {
  it("timerfd_interrupt renders a timerfd with the given interval + env", () => {
    ...
    expect(g.proverEnv()).toEqual({ "ZERO_RACE_WIDEN_TIMERFD_NS": "25000" });
  });
  ...
  it("epoll_waitqueue_flood renders the loop and scales flood threads", () => {
    ...
    expect(env["ZERO_RACE_WIDEN_EPOLL_ITEMS"]).toBe("50000");
    // ceil(50000/4096)=13 threads, clamped to <=64.
    expect(Number(env["ZERO_RACE_FLOOD_THREADS"])).toBe(13);
  });
  ...
  it("cache_miss_stall strides the buffer and pins same-CPU", () => {
    ...
    const env = g.proverEnv();
    expect(env["ZERO_RACE_WIDEN_CACHE_KB"]).toBe("8192");
    expect(env["ZERO_RACE_WIDEN_CACHE_STRIDE"]).toBe("128");
    expect(env["ZERO_RACE_SAME_CPU"]).toBe("1");
  });
  ...
  it("mutex_sleep_widen maps holdUs to nanosleep + PARK_US", () => {
    ...
    expect(g.proverEnv()).toEqual({ "ZERO_RACE_PARK_US": "300" });
  });
  ...
  it("futex_hold renders a futex wait with the hold time", () => {
    ...
    expect(g.proverEnv()).toEqual({ "ZERO_RACE_WIDEN_FUTEX_US": "750" });
  });
...
describe("composeGadgetSetup", () => {
  it("concatenates C in order and merges prover env", () => {
    ...
    expect(composed.proverEnv["ZERO_RACE_WIDEN_CACHE_KB"]).toBeDefined();
    expect(composed.proverEnv["ZERO_RACE_WIDEN_TIMERFD_NS"]).toBeDefined();
    expect(composed.proverEnv["ZERO_RACE_WIDEN_EPOLL_ITEMS"]).toBeDefined();
  });
  ...
  it("later gadgets win on env key clashes", () => {
    ...
    expect(composed.proverEnv["ZERO_RACE_PARK_US"]).toBe("900");
  });
});
...
describe("prover glue helpers", () => {
  ...
  it("buildWidenEnv formats the widen env; empty for no widen", () => {
    ...
    expect(buildWidenEnv({ symbol: "ep_poll", offset: 0x1c }, 7)).toEqual({
      "ZERO_KERNEL_QEMU_WIDEN_SYMBOL": "ep_poll",
      "ZERO_KERNEL_QEMU_WIDEN_OFFSET": "0x1c",
      "ZERO_KERNEL_QEMU_WIDEN_DELAY_MS": "7",
    });
  });
  ...
  it("setEnv sets then restores prior values", () => {
    const KEY = "ZERO_TEST_RACE_KEY_XYZ";
  ...
  it("splices gadgets, carries widen+gadget env, and maps the verdict", async () => {
    ...
    const prover = makeKernelVmRaceProver({
      ...
      verify: async (opts) => {
        capturedOpts = opts;
        envAtVerify = {
          cacheKb: process.env["ZERO_RACE_WIDEN_CACHE_KB"],
          widenSym: process.env["ZERO_KERNEL_QEMU_WIDEN_SYMBOL"],
          widenDelay: process.env["ZERO_KERNEL_QEMU_WIDEN_DELAY_MS"],
        };
        return {
          status: "reproduced",
          signature: "kasan-uaf",
    ...
    expect(process.env["ZERO_KERNEL_QEMU_WIDEN_SYMBOL"]).toBeUndefined();
...
describe("ExpRace real-IPI widener gadgets", () => {
  it("reschedule_ipi renders the sched_setaffinity IPI source and is CONFIG_PREEMPT-gated", () => {
    ...
    expect(g.proverEnv()["ZERO_RACE_WIDEN_RESCHED_BOUNCES"]).toBe("500");
  });
  ...
  it("waitqueue_freeze renders calif's timerfd/epoll freeze", () => {
    ...
    expect(g.proverEnv()["ZERO_RACE_WIDEN_WAITQUEUE_ENTRIES"]).toBe("12345");
  });
  ...
  it("retry_until_splat carries only env budget (loop lives in the harness)", () => {
    ...
    expect(g.proverEnv()["ZERO_RACE_RETRIES"]).toBe("5000");
    expect(g.proverEnv()["ZERO_RACE_SECONDS"]).toBe("25");
