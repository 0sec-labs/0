 ...
 *      knobs it wants (`ZERO_RACE_*` / `ZERO_KERNEL_QEMU_WIDEN_*`).
 ...
 * `proverEnv()` emits the `ZERO_RACE_*` / `ZERO_KERNEL_QEMU_WIDEN_*` knobs
...
export function timerfdInterruptGadget(params: { intervalNs?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_WIDEN_TIMERFD_NS": String(intervalNs) };
    },
  };
}
...
export function epollWaitqueueFloodGadget(params: { items?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return {
        "ZERO_RACE_WIDEN_EPOLL_ITEMS": String(items),
        "ZERO_RACE_FLOOD_THREADS": String(floodThreads),
      };
    },
  };
}
...
export function cacheMissStallGadget(
  ...
  return {
    ...
    proverEnv() {
      return {
        "ZERO_RACE_WIDEN_CACHE_KB": String(footprintKb),
        "ZERO_RACE_WIDEN_CACHE_STRIDE": String(strideBytes),
        // Cache eviction is most effective when the racer is pinned same-CPU.
        "ZERO_RACE_SAME_CPU": "1",
      };
    },
  };
}
...
export function mutexSleepWidenGadget(params: { holdUs?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_PARK_US": String(holdUs) };
    },
  };
}
...
export function futexHoldGadget(params: { holdUs?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_WIDEN_FUTEX_US": String(holdUs) };
    },
  };
}
...
// no debug gate. Each maps 1:1 to a `renderSetup()` C snippet + `ZERO_RACE_*`
...
export function rescheduleIpiGadget(
  ...
  return {
    ...
    proverEnv() {
      return {
        "ZERO_RACE_WIDEN_RESCHED_BOUNCES": String(bounces),
        "ZERO_RACE_SAME_CPU": "0",
      };
    },
  };
}
...
export function tlbShootdownIpiGadget(params: { flips?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_WIDEN_TLB_FLIPS": String(flips) };
    },
  };
}
...
export function membarrierIpiGadget(params: { count?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_WIDEN_MEMBARRIER": String(count) };
    },
  };
}
...
export function waitqueueFreezeGadget(params: { entries?: number } = {}): RaceGadget {
  ...
  return {
    ...
    proverEnv() {
      return { "ZERO_RACE_WIDEN_WAITQUEUE_ENTRIES": String(entries) };
    },
  };
}
 ...
 * `ZERO_RACE_*` budget knobs the harness reads.
 */
export function retryUntilSplatGadget(
  ...
  return {
    ...
    proverEnv() {
      return {
        "ZERO_RACE_RETRIES": String(retries),
        "ZERO_RACE_SECONDS": String(seconds),
      };
    },
  };
}
...
export const GADGET_SETUP_MARKER = "// ZERO_RACE_GADGET_SETUP";
...
/** Build the `ZERO_KERNEL_QEMU_WIDEN_*` env for an mdelay-kprobe widen. Pure. */
export function buildWidenEnv(widen: WidenSpec | undefined, delayMs: number): Record<string, string> {
  if (!widen) return {};
  return {
    "ZERO_KERNEL_QEMU_WIDEN_SYMBOL": widen.symbol,
    "ZERO_KERNEL_QEMU_WIDEN_OFFSET": `0x${widen.offset.toString(16)}`,
    "ZERO_KERNEL_QEMU_WIDEN_DELAY_MS": String(delayMs),
  };
}
 ...
 * gadget C into the base reproducer, sets the widen (`ZERO_KERNEL_QEMU_WIDEN_*`)
 * and gadget (`ZERO_RACE_*`) env, and runs `verifyKernelFinding` (build-cached
