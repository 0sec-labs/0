...
describe("oastConfirmedPayload", () => {
  const verified: OracleResult = {
    verified: true,
    confidence: 1,
    evidence: "DNS callback: host=abc.oast.0.ai",
    reason: "",
  };
  ...
  it("is independent of ZERO_FEATURE_POV_GATE (reads no feature flag)", () => {
    const prev = process.env["ZERO_FEATURE_POV_GATE"];
    delete process.env["ZERO_FEATURE_POV_GATE"]; // pov_gate OFF (default)
    const p = oastConfirmedPayload(makeFinding({ id: "f", category: "ssrf" }), verified);
    expect(p?.oracle).toBe("oast-callback"); // still emits
    if (prev === undefined) delete process.env["ZERO_FEATURE_POV_GATE"];
    else process.env["ZERO_FEATURE_POV_GATE"] = prev;
  });
});
