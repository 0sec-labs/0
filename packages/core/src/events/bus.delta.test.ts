...
describe("eventBus.emit('delta', …)", () => {
  ...
  it("cloudEventSink projects a delta event onto a single ZERO_EVENT_DELTA stdout line", () => {
    ...
    try {
      ...
      expect(line.startsWith("ZERO_EVENT_DELTA ")).toBe(true);
      const json = JSON.parse(line.slice("ZERO_EVENT_DELTA ".length));
