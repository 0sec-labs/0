...
describe("ToolExecutor.done — anti-honeypot integration", () => {
  const originalEnv = process.env["ZERO_FEATURE_DECOY_DETECTION"];
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env["ZERO_FEATURE_DECOY_DETECTION"] = originalEnv;
    }
  });
  ...
  it("passes the decoy through when ZERO_FEATURE_DECOY_DETECTION=0", async () => {
