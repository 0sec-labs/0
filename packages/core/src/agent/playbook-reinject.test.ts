...
afterEach(() => {
  delete process.env["ZERO_FEATURE_DYNAMIC_PLAYBOOKS"];
  delete process.env["ZERO_FEATURE_PRESERVE_CRITICAL_MESSAGES"];
});
...
describe("dynamic playbook injection survives compaction", () => {
  it("injects once, then restores the block after a real compaction erases it", async () => {
    ...
    process.env["ZERO_FEATURE_PRESERVE_CRITICAL_MESSAGES"] = "0";
