/**
 * Feature presets — named bundles of `ZERO_FEATURE_*` env vars.
 *
 * ## Why this exists
 *
 ...
 *   - `ZERO_FEATURE_LEARNED_ROUTER` and `ZERO_FEATURE_DYNAMIC_TRIAGE` —
 ...
 *   - `ZERO_FEATURE_INLINE_VALIDATION` — it runs inside the attack loop and
...
/**
 * Apply the preset named by `ZERO_FEATURE_PRESET`, if any. Returns
...
export function applyFeaturePresetFromEnv(
  ...
  const raw = env["ZERO_FEATURE_PRESET"];
