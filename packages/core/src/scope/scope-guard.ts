...
export interface ScopeGuardStatus {
  ...
  active: boolean;
  /** True when `ZERO_REQUIRE_SCOPE` opts this run into fail-closed. */
  required: boolean;
...
/**
 * Opt-in strictness switch. `ZERO_REQUIRE_SCOPE=1` turns the missing-scope
 ...
 * configuration through `ZERO_*` env vars, so an env knob is the only way
 ...
 * `ZERO_REQUIRE_SCOPE` is set and no scope is configured.
 */
export function scopeRequiredRefusal(site: string): string {
  return (
    `${site} refused: ZERO_REQUIRE_SCOPE is set but no engagement scope is configured. ` +
    ...
    "Pass --scope <file>, or unset ZERO_REQUIRE_SCOPE to run in fail-loud mode."
  );
}
