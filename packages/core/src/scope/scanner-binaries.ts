// ── Scanner-binary blacklist (0#217) ──
//
...
// forbidden.
//
// 0 is a shell-first agent (not a scanner) by design. The agent
...
// when scope is loaded (0#215), the agent is presumed to be
...
function detectInSegment(tokens: string[]): ScannerDetection | null {
  ...
  if (SCANNER_BINARIES.includes(headBase)) {
    return {
      binary: headBase,
      reason: `'${headBase}' is a generic vulnerability scanner. Coordinated-disclosure scope policies typically forbid it (0#217). Use the agent's manual probe tools (http_request, crawl) instead, or pass --allow-scanners to override.`,
    };
  }
  ...
  if (headBase === "nmap") {
    for (let j = i + 1; j < tokens.length; j++) {
      ...
      if (NMAP_FORBIDDEN_FLAGS.has(tok)) {
        return {
          binary: `nmap ${tok}`,
          reason: `'nmap ${tok}' performs service/OS fingerprinting and matches the generic-scanner traffic pattern that coordinated-disclosure scope policies forbid (0#217). Plain port scans are unaffected. Pass --allow-scanners to override.`,
        };
      }
    }
    return null;
    ...
    for (let j = i + 1; j < tokens.length - 1; j++) {
      if (tokens[j] === "-m") {
        ...
        if (mod && SCANNER_BINARIES.includes(mod)) {
          return {
            binary: `python -m ${mod}`,
            reason: `'python -m ${mod}' invokes the same generic scanner via its Python module entry point. Coordinated-disclosure scope policies typically forbid it (0#217). Pass --allow-scanners to override.`,
          };
        }
      }
    }
