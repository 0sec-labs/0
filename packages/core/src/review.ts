...
function resolveRepo(
  ...
  // Clone the repo
  const tempDir = join(tmpdir(), `0-review-${randomUUID().slice(0, 8)}`);
...
async function runReviewAgent(
  ...
  if (profile === "linux-kernel") {
    ...
    // Cross-subsystem data-flow context (0#469). Computed in the seed
...
export async function sourceReview(
  ...
  try {
    // Step 2: static scanner scan — scoped to subsystem when set (0#466)
    ...
    // Cross-subsystem data-flow prompt block (0#469); populated below for
    ...
    if ((config.profile ?? "default") === "linux-kernel") {
      ...
      // Step 2d: cross-subsystem data-flow seed (0#469). Copy Fail and its
      ...
      // Step 2e: auto-anchor from recent fix commits (0#469 / Naptime
