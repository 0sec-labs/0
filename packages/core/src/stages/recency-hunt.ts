...
export interface RecencySurvivor {
  ...
  /**
   * A bug-spec + trigger-seed shaped for `0 exploit --autoclimb` and the
...
function shapeSurvivor(
  ...
  return {
    ...
    bugSpec: {
      ...
      analysis,
      nextSteps: [
        `0 exploit --autoclimb --source <tree> --target ${rec.file} (weaponize this ${detector} lead)`,
  ...
  return {
    ...
    bugSpec: {
      ...
      nextSteps: [
        ...
        `Weaponize from the witnessing PoC: 0 exploit --autoclimb --source <tree> --target ${rec.file}`,
        `If weaponized: stage via the disclosure stager — operator-gated, do NOT auto-send`,
      ],
    },
  };
