 ...
 * corpus (`@0/benchmark`'s `hunt-variant-v1.jsonl`, read by PATH — core
 * must not depend on `@0/benchmark`) — rows where the skeptic+prover gate
 ...
 * deployment with no corpus is exactly zero. Set `ZERO_HUNT_NEGATIVES=0` to
...
export function huntNegativesEnabled(): boolean {
  const raw = process.env["ZERO_HUNT_NEGATIVES"];
 ...
 * configured.
 *
 * `HUNT_CORPUS_PATH` is the same env var `@0/benchmark`'s
 ...
 * package-relative default lives inside `@0/benchmark` and guessing at it
