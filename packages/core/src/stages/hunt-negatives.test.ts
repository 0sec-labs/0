 ...
 *     with ZERO_HUNT_NEGATIVES=1); at most ONE negative is ever attached; a
 *     novel finding's prompt is unaffected; ZERO_HUNT_NEGATIVES=0 restores
...
describe("huntNegativesEnabled", () => {
  it("is ON by default and OFF only for an explicit falsey ZERO_HUNT_NEGATIVES", () => {
    const prev = process.env["ZERO_HUNT_NEGATIVES"];
    try {
      delete process.env["ZERO_HUNT_NEGATIVES"];
      expect(huntNegativesEnabled()).toBe(true);
      process.env["ZERO_HUNT_NEGATIVES"] = "no";
      expect(huntNegativesEnabled()).toBe(false);
      process.env["ZERO_HUNT_NEGATIVES"] = "0";
      expect(huntNegativesEnabled()).toBe(false);
      process.env["ZERO_HUNT_NEGATIVES"] = "";
      expect(huntNegativesEnabled()).toBe(false);
      process.env["ZERO_HUNT_NEGATIVES"] = "1";
      expect(huntNegativesEnabled()).toBe(true);
    } finally {
      if (prev === undefined) delete process.env["ZERO_HUNT_NEGATIVES"];
      else process.env["ZERO_HUNT_NEGATIVES"] = prev;
    }
  });
});
...
describe("makeSkepticVerifier — learned-negatives wiring", () => {
  it("attaches negative context to the prompt for a matching finding when ZERO_HUNT_NEGATIVES=1, but still calls the finder and honors its outcome", async () => {
    const prev = process.env["ZERO_HUNT_NEGATIVES"];
    process.env["ZERO_HUNT_NEGATIVES"] = "1";
    try {
