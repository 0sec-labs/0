 ...
 *     (`@0/benchmark`'s `hunt-variant-v1.jsonl`, read by PATH here — core
 *     must not depend on `@0/benchmark`, the wrong dependency direction)
 ...
 *
 * Three operations, all opt-in (`ZERO_HUNT_FLYWHEEL=1`, default OFF):
 *
...
export function huntFlywheelEnabled(): boolean {
  return !["", "0", "false", "no"].includes((process.env["ZERO_HUNT_FLYWHEEL"] ?? "").toLowerCase());
}
...
// Read by path, not import: `@0/core` must not depend on
// `@0/benchmark` (hunt-corpus.ts there depends on `@0/core`, not the
...
export class HuntMemory {
   ...
   * `HuntFindingRecord` — the same shape `@0/benchmark`'s
