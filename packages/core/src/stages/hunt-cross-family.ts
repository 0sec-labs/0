 ...
 * module header). Set `ZERO_HUNT_CROSS_FAMILY=0` to pin the old correlated
 * behaviour for an ablation.
 */
export function crossFamilyRefuteEnabled(): boolean {
  const raw = process.env["ZERO_HUNT_CROSS_FAMILY"];
 ...
 * imported so this module keeps its `@0/shared`-only dependency and stays
 ...
 *     ids via ZERO_HUNT_REFUTER_CANDIDATES, which {@link refuterFamily}
...
const REFUTER_ROSTER: ReadonlyArray<{ model: string; envKeys: readonly string[] }> = [
  ...
  { model: "gpt-5.5", envKeys: ["ZERO_CHATGPT_ACCESS_TOKEN", "ZERO_CHATGPT_OAUTH_REFRESH_TOKEN", "OPENAI_API_KEY"] },
 ...
 *
 * `ZERO_HUNT_REFUTER_CANDIDATES` (comma-separated model ids) overrides the
...
export function availableRefuterCandidates(): string[] {
  const override = process.env["ZERO_HUNT_REFUTER_CANDIDATES"];
