 ...
 * `ZERO_KERNEL_QEMU_WIDEN_*` prover knobs, and the runHuntScan-plug mapping.
 *
...
describe("generateRaceSmellCandidates", () => {
  ...
  it("maps the widenHint onto the ZERO_KERNEL_QEMU_WIDEN_* prover knobs (model delay, then default)", async () => {
    const plan = await generateRaceSmellCandidates({
      sourceRoot: "/src",
      subsystemFiles: ["crypto/af_alg.c", "fs/pipe.c"],
      runtime: "api",
    ...
    expect(plan.widenEnvs[0]).toEqual({
      "ZERO_KERNEL_QEMU_WIDEN_SYMBOL": "af_alg_sendmsg",
      "ZERO_KERNEL_QEMU_WIDEN_DELAY_MS": "800",
    });
    // candidate 1 omitted it -> default 500
    expect(plan.widenEnvs[1]).toEqual({
      "ZERO_KERNEL_QEMU_WIDEN_SYMBOL": "pipe_write",
      "ZERO_KERNEL_QEMU_WIDEN_DELAY_MS": "500",
    });
  });
  ...
  it("respects a caller-supplied defaultWidenDelayMs for candidates without a suggestion", async () => {
    ...
    expect(plan.widenEnvs[1]["ZERO_KERNEL_QEMU_WIDEN_DELAY_MS"]).toBe("1200");
  });
...
describe("widenEnvFor", () => {
  ...
  it("uses the suggested delay when positive", () => {
    ...
    expect(env).toEqual({ "ZERO_KERNEL_QEMU_WIDEN_SYMBOL": "fn", "ZERO_KERNEL_QEMU_WIDEN_DELAY_MS": "250" });
  });
  it("falls back to the default when the suggested delay is missing or non-positive", () => {
    expect(widenEnvFor({ ...base, widenHint: { injectSymbol: "fn", rationale: "r" } }, 500)["ZERO_KERNEL_QEMU_WIDEN_DELAY_MS"]).toBe("500");
    expect(widenEnvFor({ ...base, widenHint: { injectSymbol: "fn", suggestedDelayMs: 0, rationale: "r" } }, 700)["ZERO_KERNEL_QEMU_WIDEN_DELAY_MS"]).toBe("700");
  });
});
