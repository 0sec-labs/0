 ...
 * (0#656). The JSON corpora live next to this module in both src (tests)
...
/** Absolute path to the frozen labeled corpus v1 (0#657). */
export function corpusV1Path(): string {
  return fileURLToPath(new URL("./corpus-v1.json", import.meta.url));
}

/** Absolute path to the references-only example manifest (0#556). */
