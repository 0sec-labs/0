...
describe("agenticScan: scan_completed emission", () => {
  ...
  beforeEach(() => {
    ...
    originalCodexLiveTargets = process.env["ZERO_FEATURE_CODEX_LIVE_TARGETS"];
    originalChatGptCodexRefreshToken = process.env["ZERO_CHATGPT_OAUTH_REFRESH_TOKEN"];
    delete process.env["ZERO_FEATURE_CODEX_LIVE_TARGETS"];
  ...
  afterEach(() => {
    ...
    if (originalCodexLiveTargets === undefined) {
      ...
      process.env["ZERO_FEATURE_CODEX_LIVE_TARGETS"] = originalCodexLiveTargets;
    }
    if (originalChatGptCodexRefreshToken === undefined) {
      delete process.env["ZERO_CHATGPT_OAUTH_REFRESH_TOKEN"];
    } else {
      process.env["ZERO_CHATGPT_OAUTH_REFRESH_TOKEN"] = originalChatGptCodexRefreshToken;
    }
    vi.restoreAllMocks();
  });
  ...
  it("does not re-enable the removed Codex MCP live runner when the old feature flag is set", async () => {
    process.env["ZERO_FEATURE_CODEX_LIVE_TARGETS"] = "1";
...
describe("agenticScan: planner LLM error mid-scan", () => {
  ...
  beforeEach(() => {
    ...
    originalSkipBanner = process.env["ZERO_SKIP_PROVIDER_BANNER"];
  ...
  afterEach(() => {
    ...
    if (originalSkipBanner === undefined) {
      delete process.env["ZERO_SKIP_PROVIDER_BANNER"];
    } else {
      process.env["ZERO_SKIP_PROVIDER_BANNER"] = originalSkipBanner;
    }
    vi.restoreAllMocks();
  });
