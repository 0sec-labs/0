...
// live getter so each test can flip ZERO_FEATURE_PUBLISHABILITY_GATE and have
    ...
    features: new Proxy(actual.features, {
      get(target, prop, receiver) {
        if (prop === "publishabilityGate") {
          const v = process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"];
...
beforeEach(() => {
  ...
  savedPerItem = process.env["ZERO_FEATURE_PER_ITEM_ORCHESTRATION"];
  process.env["ZERO_FEATURE_PER_ITEM_ORCHESTRATION"] = "0";
  savedApiKey = process.env.ANTHROPIC_API_KEY;
  savedFlag = process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"];
...
afterEach(() => {
  ...
  if (savedPerItem === undefined) delete process.env["ZERO_FEATURE_PER_ITEM_ORCHESTRATION"];
  else process.env["ZERO_FEATURE_PER_ITEM_ORCHESTRATION"] = savedPerItem;
  if (savedApiKey === undefined) delete process.env.ANTHROPIC_API_KEY;
  else process.env.ANTHROPIC_API_KEY = savedApiKey;
  if (savedFlag === undefined) delete process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"];
  else process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"] = savedFlag;
  ...
  const cacheFile = join(homedir(), ".0", "intel-cache", "osv-query", `${digest}.json`);
...
describe("runPipeline — novelty gate on OSS package scans (#851)", () => {
  it("stamps noveltyVerdict + advisoryMatches on a confirmed npm-package finding", async () => {
    process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"] = "1";
  ...
  it("leaves the verdict unset when the publishability gate flag is off", async () => {
    delete process.env["ZERO_FEATURE_PUBLISHABILITY_GATE"];
