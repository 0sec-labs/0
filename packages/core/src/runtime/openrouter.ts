...
export class OpenRouterRuntime implements NativeRuntime {
  ...
  constructor(config: OpenRouterConfig = {}) {
    ...
    if (config.ensembleModels && config.ensembleModels.length > 0) {
      ...
      // Single model mode
      const model = config.model ?? process.env["ZERO_MODEL"] ?? "anthropic/claude-sonnet-4.6";
  ...
  get models(): string[] {
    ...
    return [this.config.model ?? process.env["ZERO_MODEL"] ?? "anthropic/claude-sonnet-4.6"];
  }
