/**
 * 0#193 / 0-cloud#111 — VerificationSpec evaluator tests.
 *
 * Coverage:
...
beforeAll(() => {
  repoRoot = mkdtempSync(join(tmpdir(), "0-verify-"));
...
function createGitDiffFixture(): {
  ...
  const root = mkdtempSync(join(tmpdir(), "0-verify-git-"));
  try {
    ...
    execFileSync(
      ...
      [
        ...
        "user.name=0 Test",
        "-c",
        "user.email=0-test@example.invalid",
        "commit",
        "--quiet",
        "-m",
        "base",
...
/**
 * 0#193 follow-up — defence-in-depth against symlink traversal.
 *
...
describe("evaluateVerificationSpec — symlink traversal guards", () => {
  ...
  beforeAll(() => {
    ...
    outsideDir = mkdtempSync(join(tmpdir(), "0-verify-outside-"));
    writeFileSync(
      join(outsideDir, "secrets.env"),
      "SECRET_KEY=should-never-be-readable\n",
    );

    symlinkRoot = mkdtempSync(join(tmpdir(), "0-verify-symlink-"));
...
/**
 * 0#193 follow-up — ReDoS + oversized-input guards.
 *
...
describe("evaluateVerificationSpec — ReDoS + oversize guards", () => {
  ...
  it("refuses to read files larger than the byte cap", async () => {
    ...
    const big = mkdtempSync(join(tmpdir(), "0-verify-big-"));
    try {
