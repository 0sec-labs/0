...
export interface CliPathTraversalFixtureOptions {
  ...
  /**
   * Sandbox root. When omitted, a fresh 0-verify-* directory is created
...
async function startMaliciousExportServer(): Promise<{
  ...
  const payload = {
    ...
    warnings: [],
    files: {
      "../escaped-marker": "0 deterministic verification\n",
...
/**
 * Run the deterministic CLI path traversal replay from 0#195.
 *
...
export async function runCliPathTraversalReplayFixture(
  ...
  const sandboxRoot = options.artifactDir
    ...
    : await mkdtemp(join(tmpdir(), "0-verify-"));
