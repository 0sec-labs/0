...
describe("verifyAgainstRef (filesystem)", () => {
  ...
  beforeAll(() => {
    repoPath = mkdtempSync(join(tmpdir(), "0-canary-"));
...
describe("verifyAgainstRef (git-backed)", () => {
  ...
  beforeAll(() => {
    repoPath = mkdtempSync(join(tmpdir(), "0-canary-git-"));
    execFileSync("git", ["init", "-q"], { cwd: repoPath });
    execFileSync("git", ["config", "user.email", "test@0.test"], { cwd: repoPath });
    execFileSync("git", ["config", "user.name", "0-test"], { cwd: repoPath });
