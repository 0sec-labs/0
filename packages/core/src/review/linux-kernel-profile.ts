...
Find a real, exploitable kernel bug in this tree. Not a theoretical issue — a defect that could earn a CVE. Your output is a hypothesis backed by a code citation and a reproducer shape; final exploit reproduction lives in 0#271 (kernel-oracle verifier) and 0#272 (syzkaller harness scaffold), not here.
...
Static-only findings (no reproducer shape attached) MUST be flagged \`confidence: 0.4\` and labelled \`hypothesis: true\`. Once 0#271 (kernel oracle) lands, a separate verification phase will promote hypotheses to confirmed findings.
