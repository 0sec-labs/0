 ...
 *   ZERO_CLOUD_SCAN_ID  — scan correlation id (sent in X-0-Scan-Id header
...
export interface CloudSinkConfig {
  ...
  sinkUrl: string;
  /** Scan correlation id used in the URL path AND the X-0-Scan-Id header */
  scanId: string;
  /** Optional bearer token sent as Authorization header */
  token?: string;
  /**
   * Optional owning-org id. Forwarded as `X-0-Org-Id` on requests that
   ...
   * which is correct for single-tenant Phase-1 operation. See 0#768.
   */
  orgId?: string;
}
...
function buildHeaders(config: CloudSinkConfig): Record<string, string> {
  ...
  if (config.orgId) headers["X-0-Org-Id"] = config.orgId;
  return headers;
}
  ...
  try {
    ...
    if (!res.ok) {
      ...
      process.stderr.write(
        `[0 cloud-sink] ${kind} POST ${url} returned ${res.status}: ${text.slice(0, 200)}\n`,
      );
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[0 cloud-sink] ${kind} POST ${url} failed: ${msg}\n`);
  }
}
 ...
 * into the strict `CloudSinkFinding` shape the 0-cloud orchestrator's
...
export function normalizeFinding(rawFinding: unknown): CloudSinkFinding {
  ...
  // 0#170 — pass-through optional PoC step graph. We accept already-
  ...
  // 0#193 — pass-through optional VerificationSpec. Same wire-shape
 ...
 * (0#193). Returns the object shape unchanged when input is already an
...
export async function postFinding(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[0 cloud-sink] dropping malformed finding: ${msg}\n`);
    return;
  }
