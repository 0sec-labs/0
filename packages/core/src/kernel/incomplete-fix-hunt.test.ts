...
describe("kernel/incomplete-fix-hunt", () => {
  ...
  beforeAll(() => {
    repo = mkdtempSync(join(tmpdir(), "0-incfix-"));
    git(repo, ["init", "-q"]);
  ...
  it("fails soft on a non-git tree", () => {
    const notGit = mkdtempSync(join(tmpdir(), "0-incfix-soft-"));
    try {
...
describe("kernel/incomplete-fix-hunt — bad-fix (fix-of-fix) ingest", () => {
  ...
  beforeAll(() => {
    repo = mkdtempSync(join(tmpdir(), "0-badfix-"));
    git(repo, ["init", "-q"]);
    git(repo, ["config", "user.email", "test@example.com"]);
    git(repo, ["config", "user.name", "Test"]);
    git(repo, ["config", "commit.gpgsign", "false"]);
  ...
  it("fails soft on a non-git tree", () => {
    const notGit = mkdtempSync(join(tmpdir(), "0-badfix-soft-"));
    try {
