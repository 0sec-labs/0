...
export const WINDOWS_ATTEMPT_SCHEMA = "0.windows-research-attempt/v1" as const;
export const WINDOWS_LEDGER_SCHEMA = "0.windows-research-ledger/v1" as const;
...
export interface WindowsResearchAttemptInput {
  ...
  repoShas: { zeroverse: string; "0": string; zeroCloud: string };
...
export interface WindowsResearchSummary {
  schemaVersion: "0.windows-research-summary/v1";
  rows: number;
...
const IMPORT_VERDICT_SCHEMA = "0.windows-hyperv-import-verdict/v1";
...
function validateInputShape(value: unknown): asserts value is WindowsResearchAttemptInput {
  ...
  const repoShas = exact(top.repoShas, "repoShas", ["zeroverse", "0", "zeroCloud"]);
...
function parseImportVerdict(path: string): ImportVerdict {
  ...
  if (row.verdictSchema !== IMPORT_VERDICT_SCHEMA || row.passed !== true
    ...
    throw new Error("invalid 0 Hyper-V import verdict");
  }
...
export function collectWindowsResearchAttempt(
  ...
  if (importVerdict && (importVerdict.receiptSha256 !== receiptSha256
    ...
    throw new Error("live proof fields are not bound to the 0 import verdict and receipt");
  }
