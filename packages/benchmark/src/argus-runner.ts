 ...
 *
 * Runs 0 against the Argus validation benchmarks (60 Dockerized web
...
function ensureRepo(repo: string, ref: string | undefined): string {
  ...
  const slug = url
    ...
    .replace(/\.git$/, "")
    .replace(/[^\w.-]+/g, "_");
  const dest = join(tmpdir(), "0-argus-cache", slug);
...
async function runChallengeOnce(challenge: ArgusChallenge): Promise<ArgusResult> {
  ...
  try {
    let report: any;
    if (useAgentic) {
      const dbPath = join(tmpdir(), `0-argus-${challenge.id}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[36m\x1b[1m  0 x Argus benchmark\x1b[0m");
