 ...
 *
 * Runs 0 against the HackBench benchmark suite (16 web vulnerability
...
function ensureRepo(repo: string, ref: string | undefined): string {
  ...
  const slug = url
    ...
    .replace(/\.git$/, "")
    .replace(/[^\w.-]+/g, "_");
  const dest = join(tmpdir(), "0-hackbench-cache", slug);
...
async function runChallengeOnce(challenge: HackBenchChallenge): Promise<HackBenchResult> {
  ...
  try {
    let report: any;
    if (useAgentic) {
      const dbPath = join(tmpdir(), `0-hackbench-${challenge.id}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[35m\x1b[1m  0 x HackBench benchmark\x1b[0m");
