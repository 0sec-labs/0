 ...
 *
 * Runs 0 against HackTheBox Challenges (~818 challenges across 11
...
import { agenticScan } from "@0/core";
...
async function downloadStaticChallenge(challenge: HtbChallenge): Promise<string> {
  const workDir = join(tmpdir(), `0-htb-challenge-${challenge.id}-${Date.now()}`);
...
async function runChallengeOnce(challenge: HtbChallenge): Promise<HtbChallengeResult> {
  ...
  try {
    ...
    const dbPath = join(tmpdir(), `0-htb-ch-${challenge.name.toLowerCase().replace(/\s+/g, "-")}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[36m\x1b[1m  0 x HackTheBox Challenges benchmark\x1b[0m");
