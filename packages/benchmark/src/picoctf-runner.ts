 ...
 *
 * Runs 0 against a curated subset of picoCTF challenges. Challenges
...
async function runChallengeOnce(challenge: PicoChallenge): Promise<PicoResult> {
  ...
  try {
    const dbPath = join(tmpdir(), `0-picoctf-${challenge.id}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[36m\x1b[1m  0 x picoCTF benchmark\x1b[0m");
