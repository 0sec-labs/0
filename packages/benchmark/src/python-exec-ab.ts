...
export interface PythonExecAbCell {
  label: "baseline" | "python-exec";
  pythonExec: boolean;
  challenges: number;
  passed: number;
  flags: number;
...
export interface PythonExecAbReport {
  timestamp: string;
  runner: "xbow";
  xbowArgs: string[];
  baseline: PythonExecAbCell;
  pythonExec: PythonExecAbCell;
  delta: {
    passed: number;
    flags: number;
...
export function compareCells(
  baseline: PythonExecAbCell,
  pythonExec: PythonExecAbCell,
): PythonExecAbReport["delta"] {
  return {
    passed: pythonExec.passed - baseline.passed,
    flags: pythonExec.flags - baseline.flags,
    passRatePctPoints: (pythonExec.passRate - baseline.passRate) * 100,
    flagRatePctPoints: (pythonExec.flagRate - baseline.flagRate) * 100,
    totalAttackTurns: pythonExec.totalAttackTurns - baseline.totalAttackTurns,
    totalEstimatedCostUsd: pythonExec.totalEstimatedCostUsd - baseline.totalEstimatedCostUsd,
    totalTokens: pythonExec.totalTokens - baseline.totalTokens,
  };
}

function runXbowCell(label: PythonExecAbCell["label"], pythonExec: boolean, xbowArgs: string[]): PythonExecAbCell {
  const __dirname = dirname(fileURLToPath(import.meta.url));
  const repoRoot = join(__dirname, "../../..");
  const result = spawnSync(
    "pnpm",
    ["--filter", "@0/benchmark", "xbow", ...xbowArgs],
    {
      cwd: repoRoot,
      encoding: "utf8",
      env: {
        ...process.env,
        "ZERO_FEATURE_PYTHON_EXEC": pythonExec ? "1" : "0",
      },
    },
  );
  ...
  return summarizeBenchmarkReport(label, pythonExec, parseBenchmarkJson(result.stdout ?? ""));
}
...
async function main(): Promise<void> {
  ...
  if (runner !== "xbow") {
    throw new Error("python-exec A/B currently supports --runner xbow only");
  }

  const jsonOutput = rawArgs.includes("--json");
  const xbowArgs = buildXbowArgs(rawArgs);
  const baseline = runXbowCell("baseline", false, xbowArgs);
  const pythonExec = runXbowCell("python-exec", true, xbowArgs);
  const report: PythonExecAbReport = {
    timestamp: new Date().toISOString(),
    runner: "xbow",
    xbowArgs,
    baseline,
    pythonExec,
    delta: compareCells(baseline, pythonExec),
  };
  ...
  console.log(`python-exec: ${pythonExec.passed}/${pythonExec.challenges} pass, ${pythonExec.flags}/${pythonExec.challenges} flags, ${pythonExec.totalAttackTurns} turns, ${pythonExec.totalTokens} tokens, $${pythonExec.totalEstimatedCostUsd.toFixed(2)}`);
  console.log("");
  ...
  console.log(`python rates:    pass ${formatRate(pythonExec.passRate)}, flags ${formatRate(pythonExec.flagRate)}`);
}
