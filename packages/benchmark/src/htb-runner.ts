 ...
 *
 * Runs 0 against HackTheBox Starting Point machines (~25 introductory
...
async function runMachineOnce(machine: HtbMachine): Promise<HtbResult> {
  ...
  try {
    const dbPath = join(tmpdir(), `0-htb-${machine.name.toLowerCase()}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[36m\x1b[1m  0 x HackTheBox Starting Point benchmark\x1b[0m");
