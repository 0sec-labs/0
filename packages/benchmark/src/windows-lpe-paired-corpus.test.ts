...
function capabilityFixture(): {
  ...
  const documents: [WindowsLpeEvaluatorLabels, WindowsLpeEvaluatorLabels] = [
    { schemaVersion: "0.windows-lpe-evaluator-labels/v2", corpusId: manifest.corpusId, split: "development", inventorySha256: "0".repeat(64), salt: "Qx7Nv2Lp9Ds4Hj6Bf1Mt8Yc3Za0Ue5Gi7Ro2Wk9VnAs", labels: [] },
    { schemaVersion: "0.windows-lpe-evaluator-labels/v2", corpusId: manifest.corpusId, split: "holdout", inventorySha256: "0".repeat(64), salt: "Cv3Mn8Qz1Kp6Xs9Dc2Fh7Jt0Ry4Ua5We3Li8Go1NbEk", labels: [] },
  ];
...
describe("Windows LPE paired corpus v2", () => {
  ...
  it("rejects duplicate JSON keys in bounded file-loader inputs", () => {
    const directory = mkdtempSync(resolve(tmpdir(), "0-paired-corpus-"));
    const path = resolve(directory, "duplicate.json");
    try {
      writeFileSync(path, '{"schemaVersion":"0.windows-lpe-paired-corpus/v2","schemaVersion":"shadow"}');
