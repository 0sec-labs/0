 ...
 *
 * Extracts (finding, ground_truth) pairs from 0 benchmark results.
 *
 * Sources:
 ...
 *   - 0 SQLite DB (ground truth = blind verify status)
 *
 ...
 *
 * Usage:
 *   tsx src/triage-data-collector.ts --db <path-to-0.db>
...
export interface TriageSample {
   ...
   * collected from sources that predate 0#112's instrumentation
   ...
   * 0#113: given the layer outcomes a finding accumulates, can a
...
function collectFromDb(dbPath: string): TriageSample[] {
  ...
  try {
    ...
    // array (0#112) — may be NULL on rows that predate the migration.
