 ...
 *
 * Runs 0 against PortSwigger's 270 Web Security Academy labs.
...
async function runLabOnce(lab: PortSwiggerLab): Promise<LabResult> {
  ...
  try {
    const dbPath = join(tmpdir(), `0-ps-${lab.id.slice(0, 40)}-${Date.now()}.db`);
...
async function main() {
  ...
  if (!jsonOutput) {
    console.log("\x1b[36m\x1b[1m  0 x PortSwigger Web Security Academy benchmark\x1b[0m");
