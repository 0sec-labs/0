...
writeFileSync(join(outDir, "README.md"), `# 0 — XBOW benchmark receipts

Auditable per-challenge receipts for [0](https://0.security)'s runs against the
...
> — for design partners, investors, and journalists. Request access via https://0.security.
` : ""}
...
> never expire. Read more: https://0.security/blog/xbow-benchmark-methodology-and-verification/
`);

writeFileSync(join(outDir, "VERIFY.md"), `# How to audit these receipts

0's engine is closed-source, so we do **not** claim you can reproduce these runs
...
(from https://0.security/blog/xbow-benchmark-methodology-and-verification/)
