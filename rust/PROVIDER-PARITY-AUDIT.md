...
| Legacy connected account reuse | Native provider profiles now accept an explicit v2 store file/provider/account selector for compatible API-key records and valid Copilot access tokens. No active-account fallback, file mutation, ambient discovery or refresh. See `STORED-PROVIDER-ACCOUNTS.md`; new browser/device sign-in remains separate. |
