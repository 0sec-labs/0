...
| Native state | `crates/zero-store`: SQLite sessions, command admission, owner-bound operation settlement, ordered events, budget reservation/settlement, transactional epoch recovery and schema v1/v2→v3 migration with optional activation epoch pins | Prompt/message/context/evidence storage; further schema upgrades; explicit legacy import; durable multi-process campaign accounting |
...
| Plugin runner | `crates/zero-plugin-runner`: pinned offline single-call RPC through Docker/smolvm, exact response correlation, retained leases on uncertainty; actual Node fixture passed on local Docker | Engine dispatch, bidirectional broker, persistent workers, broader backend/platform qualification |
| Generation graph | `crates/zero-harness`: verified complete plugin/artifact/policy graph, activation epoch pins, durable invocation leases and current-state rollback | Persist session activation epochs; integrate engine calls, measured evaluators and native process replacement |
...
| CLI | `crates/zero-cli`: `schema`, `snapshot pin`, `session create/list/show/events/budget/reconcile-usage`, `exec`, `sandbox`, `infer`, `agent`, line `console`, `doctor`, `app-server`, help/version; separate `.0sec/native/state.db` | All legacy commands below; UX/exit/schema compatibility; installer and platform release qualification |
