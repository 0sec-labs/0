...
fn retained_rows(conn: &Connection) -> Rows {
    let mut tables = conn
        .prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT GLOB 'sqlite_*' AND name<>'reviews' ORDER BY name")
        .unwrap();
    let names = tables
...
fn historical(conn: &Connection, version: u32) {
    conn.execute_batch("DROP INDEX review_command_created; DROP TABLE reviews;")
        .unwrap();
    if version == 16 {
