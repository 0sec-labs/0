...
fn schema_thirteen_migration_preserves_existing_campaign_snapshot() {
    ...
    sql.execute_batch("DROP INDEX native_reproduction_admission_command; DROP INDEX native_reproduction_parent_command; DROP INDEX native_reproduction_command; DROP TABLE native_reproductions; DROP INDEX source_archive_command; DROP TABLE source_archives; DROP INDEX review_command_created; DROP TABLE reviews; DROP INDEX scan_command_created; DROP TABLE scans; DROP TABLE strategy_search_selections; DROP TABLE strategy_search_evaluations; DROP TABLE strategy_search_proposals; DROP TABLE strategy_searches; DROP TABLE strategy_sessions; PRAGMA user_version=13;")
        .unwrap();
    drop(sql);
    assert!(matches!(
...
fn schema_fifteen_upgrade_preserves_fixed_pair_portable_identity() {
    ...
    let path = dir.path().join("state.db");
    let sql = rusqlite::Connection::open(&path).unwrap();
    sql.execute_batch("DROP INDEX native_reproduction_admission_command; DROP INDEX native_reproduction_parent_command; DROP INDEX native_reproduction_command; DROP TABLE native_reproductions; DROP INDEX source_archive_command; DROP TABLE source_archives; DROP INDEX review_command_created; DROP TABLE reviews; DROP INDEX scan_command_created; DROP TABLE scans; DROP TABLE strategy_search_selections; PRAGMA user_version=15;")
        .unwrap();
    drop(sql);
    assert!(matches!(
        Store::open_read_only(&path),
...
fn schema_sixteen_scan_migration_preserves_fixed_pair_evidence() {
    ...
    let path = dir.path().join("state.db");
    let sql = rusqlite::Connection::open(&path).unwrap();
    sql.execute_batch("DROP INDEX native_reproduction_admission_command; DROP INDEX native_reproduction_parent_command; DROP INDEX native_reproduction_command; DROP TABLE native_reproductions; DROP INDEX source_archive_command; DROP TABLE source_archives; DROP INDEX review_command_created; DROP TABLE reviews; DROP INDEX scan_command_created; DROP TABLE scans; PRAGMA user_version=16;")
        .unwrap();
    drop(sql);
    assert!(matches!(
        Store::open_read_only(&path),
...
fn changed_sixteen_schema_is_rejected_without_partial_scan_migration() {
    ...
    let path = dir.path().join("state.db");
    let sql = rusqlite::Connection::open(&path).unwrap();
    sql.execute_batch("DROP INDEX native_reproduction_admission_command; DROP INDEX native_reproduction_parent_command; DROP INDEX native_reproduction_command; DROP TABLE native_reproductions; DROP INDEX source_archive_command; DROP TABLE source_archives; DROP INDEX review_command_created; DROP TABLE reviews; DROP INDEX scan_command_created; DROP TABLE scans; PRAGMA user_version=16; CREATE INDEX changed_sessions ON sessions(created_at_ms);").unwrap();
    assert!(matches!(
        Store::open(&path),
