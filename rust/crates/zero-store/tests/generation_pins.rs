...
fn schema_v2_migration_keeps_unpinned_history_and_persists_distinct_activation_epochs() {
    ...
    conn.execute_batch("DROP TABLE agent_inputs; DROP TABLE operation_artifacts; DROP TABLE artifacts; ALTER TABLE sessions DROP COLUMN generation_epoch; PRAGMA user_version=2;")
        .unwrap();
    drop(conn);
    let mut store = Store::open(&path).unwrap();
