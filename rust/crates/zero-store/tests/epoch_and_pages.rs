...
fn v1_migration_recovers_all_pre_epoch_owners_and_keeps_data() {
    ...
    drop(store);
    let conn = rusqlite::Connection::open(&path).unwrap();
    conn.execute_batch("DROP TABLE agent_inputs; DROP TABLE operation_artifacts; DROP TABLE artifacts; ALTER TABLE sessions DROP COLUMN generation_epoch; DROP TABLE engine_epoch; PRAGMA user_version=1;")
        .unwrap();
    drop(conn);
    let mut store = Store::open(&path).unwrap();
