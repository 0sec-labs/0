...
use zero_protocol::strategy_registry::{
    StrategyImportRequest, StrategyImportResult, StrategySearchEvidenceDescriptor,
};
#[derive(serde::Serialize)]
pub struct StrategySearchEligibilityPreparation {
    pub evidence_sha256: String,
    pub descriptor: StrategySearchEvidenceDescriptor,
}
...
fn retained(
    ...
    let bytes = registry
        ...
        .map_err(error)?;
    let descriptor: StrategySearchEvidenceDescriptor = serde_json::from_slice(&bytes)?;
    let manifest = registry
        .artifact_bounded(&descriptor.snapshot_sha256, 1024 * 1024)
        .map_err(error)?;
    let measured = reassess_strategy_search_evidence(&manifest, |id| {
        registry
...
pub fn prepare_strategy_search_eligibility(
    source: &Path,
    registry: &Path,
    campaign: &str,
) -> Result<StrategySearchEligibilityPreparation, EngineError> {
    let evidence = export_strategy_search_evidence(source, campaign)?;
...
pub fn import_strategy_search_eligibility(
    ...
    drop(registry);
    let evidence = export_strategy_search_evidence(source, &request.campaign_id)?;
    ...
    registry
        .import_strategy_search_evidence(request, &descriptor, &artifacts, |reader| {
            let manifest = reader(&descriptor.snapshot_sha256)?;
            let measured = reassess_strategy_search_evidence(&manifest, |digest| {
                reader(digest).map_err(error)
            })
...
pub fn read_strategy_search_eligibility_receipt(
    registry_path: &Path,
    receipt: &str,
) -> Result<StrategyImportResult, EngineError> {
    let registry = Registry::open_read_only(registry_path).map_err(error)?;
...
pub fn read_strategy_search_eligibility(
    registry_path: &Path,
    request: &StrategyImportRequest,
) -> Result<StrategyImportResult, EngineError> {
    let registry = Registry::open_read_only(registry_path).map_err(error)?;
