...
pub(crate) fn effect_origin(
    ...
    let parent_id = effect.payload["parent_operation"]
        .as_str()
        .ok_or_else(|| error("frozen web verification parent absent"))?;
    let parent = store.get_operation(parent_id)?;
    let intent = store.validate_web_verification_parent(&parent)?;
    let frozen = FrozenPlan::from_intent(&intent).map_err(error)?;
    let origin = &effect.payload["origin"];
    let index = origin["case_index"]
        .as_u64()
        .and_then(|n| usize::try_from(n).ok())
        .ok_or_else(|| error("web verification case index absent"))?;
    let repeat = origin["repeat_index"]
        .as_u64()
        .and_then(|n| u32::try_from(n).ok())
        .ok_or_else(|| error("web verification repeat absent"))?;
    let request = frozen.request(index, repeat).map_err(error)?;
    let case = &frozen.plan().cases[index];
    let expected = child_payload(&parent.id, &frozen, index, repeat, &request)?;
    if effect.session_id != parent.session_id
        || effect.payload != expected
        || effect.command_id != format!("{}:web:case:{index}:{repeat}", parent.id)
        || origin["case_name"] != case.name
    {
        return Err(error("HTTP effect differs from frozen web matrix"));
    }
    let review = store.get_operation(&frozen.plan().web_operation_id)?;
    Ok((parent, review))
}
pub(super) fn child_payload(
    parent: &str,
    frozen: &FrozenPlan,
    index: usize,
    repeat: u32,
    request: &zero_protocol::http::HttpRequestIntent,
) -> Result<Value, EngineError> {
    let case = frozen
        .plan()
        .cases
        .get(index)
        .ok_or_else(|| error("web case missing"))?;
    Ok(
        json!({"kind":"agent_http","parent_operation":parent,"origin":{"kind":"frozen_web_plan","plan_sha256":frozen.plan_sha256(),"case_index":index,"case_name":case.name,"repeat_index":repeat},"http_context":frozen.intent()["http_context"],"http_output_version":2,"request":request}),
    )
}
pub(super) fn attempt(
    ...
    let mut attempt = WebVerificationAttempt {
        case_name: frozen.plan().cases[index].name.clone(),
        repeat_index: repeat,
...
fn artifact(
    ...
    let digest = attachments
        .get(name)
        .ok_or_else(|| error("web verification artifact missing"))?;
    let actual: Value = serde_json::from_slice(&store.artifact(digest)?)?;
    if &actual != expected {
        return Err(error("web verification artifact differs"));
    }
    Ok(())
}
...
pub(crate) fn report(
    ...
    source_id: &str,
    id: &str,
) -> Result<WebVerificationReport, EngineError> {
    let parent = store.get_operation(id)?;
    if parent.session_id != session {
        return Err(error("web verification belongs to another session"));
    }
    let frozen = FrozenPlan::from_intent(&store.validate_web_verification_parent(&parent)?)
        .map_err(error)?;
    if frozen.plan().web_operation_id != source_id {
        return Err(error("web verification belongs to another review"));
    }
    let checked = prepare(store, session, frozen.plan().clone())?;
    if checked.intent() != frozen.intent() {
        return Err(error("web verification original authority differs"));
    }
    ...
    let retained: Option<WebVerificationOutcome> = if recovered {
        ...
        Some(serde_json::from_value(parent.outcome.clone().ok_or_else(
            || error("web verification has no retained settlement"),
        )?)?)
    };
    if let Some(outcome) = &retained {
        if outcome.attempts.len() > 24 || outcome.artifacts != attachments {
            return Err(error("web verification matrix/artifact index differs"));
        }
        artifact(
            store,
            &parent,
    ...
    for repeat in 0..frozen.plan().repeats {
        for index in 0..frozen.plan().cases.len() {
            match store
                .get_operation_by_command(session, &format!("{id}:web:case:{index}:{repeat}"))
            {
                Ok(child) => {
                    if missing {
                        return Err(error("web verification matrix is not a prefix"));
                    }
    ...
    let outcome = if let Some(outcome) = retained {
        if serde_json::to_value(&actual)? != serde_json::to_value(&outcome.attempts)?
            ...
            || execution::status(&assessment) != parent.status
        {
            return Err(error(
                "web verification evidence/assessment/settlement differs",
            ));
        }
        outcome
    } else {
        ...
        if let Some(digest) = attachments.get("web.verification.matrix") {
            ...
            {
                return Err(error("interrupted web matrix differs"));
            }
        }
        WebVerificationOutcome {
            ...
            error: Some(
                "verification owner ended before terminal settlement; no automatic replay".into(),
            ),
        }
    };
    Ok(WebVerificationReport {
        operation_id: id.into(),
        operation_status: parent.status,
        plan: frozen.plan().clone(),
        intent_sha256: frozen.intent_sha256().into(),
        approved_intent_sha256: parent.payload["request"]["approved_intent_sha256"]
            .as_str()
            .map(str::to_owned),
        outcome,
    })
}
pub fn read_web_verification(
    path: &Path,
    session: &str,
    source_id: &str,
    id: &str,
) -> Result<WebVerificationReport, EngineError> {
    report(&Store::open_read_only(path)?, session, source_id, id)
}
