...
#[tokio::test]
async fn web_only_evidence_navigation_and_explicit_triage_are_offline_and_restore_the_terminal() {
    ...
    std::fs::write(
        ...
        json!({
            ...
            "session":session,"root":dir.path(),"policy":support::policy("http://127.0.0.1:1/target/"),
            "request":{"provider":"fixture","model":"fixture","instructions":"Investigate within the captured HTTP scope", "prompt":"Observe and submit a hypothesis", "max_turns":2,"reservation_per_turn":10,"http_profile":"target","web_submission_max_hypotheses":2}
    ...
    let (record, history) =
        zero_engine::read_web_finding(&state, &session, operation, hypothesis, 0, 50).unwrap();
    assert_eq!(record.revision, 1);
    assert_eq!(record.status, zero_protocol::web::WebTriageStatus::Accepted);
