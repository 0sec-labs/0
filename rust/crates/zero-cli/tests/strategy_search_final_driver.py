"""Physical full-history search: two proposals, explicit protected selection, source import."""
import copy
import hashlib
import http.server
import json
        ...
        try:
            ...
            if request['model']=='proposal-model':
                ...
                if n<2:
                    ...
                    feedback=payload['development_feedback']
                    assert len(feedback['evaluations'])==2,feedback
                    improved=[e for e in feedback['evaluations'] if e['improved']]
                    assert len(improved)==1,feedback
                    args={'action':'select_final','evaluation_id':improved[0]['evaluation_id'],'rationale':'Select the independently improved candidate; no claim of Final success'}
                ...
                if 'CANDIDATE_TWO' not in request['instructions']:
                    ...
                    if not results:
                        ...
                        if 'private-search-marker-' in body:
                            claims=[{'title':'Observed fixture disclosure','category':'fixture_disclosure','explanation':'Actual resource evidence','claimed_impact':'Local fixture measurement only','claimed_severity':'low','citations':[{'operation_id':ref['operation_id'],'response_manifest_sha256':ref['response_manifest_sha256'],'part':{'type':'body','offset':0,'length':len(body.encode())}}]}]
...
plan['max_proposals']=3
plan['protected_final']={'scenarios':[dict(s,id=s['id'].replace('development','final'),family=s['family'].replace('development','final'),lane='final',marker='private-search-marker-FINAL-'+str(s['positive'])) for s in plan['scenarios']],'repeats':2,'minimum_gain':1}
suite=digest(canonical({'version':'local_web_marker_v1','scenarios':sorted(plan['protected_final']['scenarios'],key=lambda s:s['id'])}))
host['strategy']['accepted_suite_sha256']=[suite]
...
try:
    ...
    for field in ['final','canary','auto_promote']:
        ...
        assert len(models)==0
    for schema,keep_final in [(1,True),(2,False)]:
        invalid=copy.deepcopy(plan);invalid['schema_version']=schema
        if not keep_final:del invalid['protected_final']
        json.dump(invalid,open(plan_file,'w'))
        invoke(['strategy','search','create','--command-id','bad-version','--plan',plan_file],True,success=False)
        assert len(models)==0
    campaign=create('normal')
    assert len(models)==0
    report=invoke(search('run',campaign),True)['report']
    assert report['schema_version']==2 and report['qualification']=='adaptive_search_fixture',report
    assert len(report['evaluations'])==2 and len(report['proposals'])==3,report
    assert [e['improved'] for e in report['evaluations']]==[False,True],report
    selected=report['selection'];final=report['final_measurement']
    assert report['proposals'][-1]['output']['action']=='select_final',report
    assert selected['evaluation_id']==report['evaluations'][1]['evaluation']['id'],selected
    assert selected['suite_sha256']==suite and selected['run_count']==8 and selected['schedule_start']==16,selected
    assert final['decision']=='improved_for_fixture_suite' and len(final['cases'])==8,final
    assert all(c['lane']=='final' for c in final['cases'])
    assert len(models)==35,(len(models),report)
    status=invoke(search('status',campaign))['snapshot']
    usage=status['campaign']['usage']
    assert usage['model_charged_micro_usd']==105 and usage['model_reserved_micro_usd']==0,usage
    assert usage['model_calls']==35 and usage['http_requests']==8,usage
    assert query('SELECT count(*) FROM campaigns')==[(1,)]
    assert query('SELECT count(*) FROM campaign_runs WHERE campaign_id=?',(campaign,))==[(24,)]
    assert query('SELECT count(*) FROM campaign_exposures')==[(1,)]
    assert query('SELECT count(*) FROM strategy_search_selections')==[(1,)]
    text=invoke(search('report',campaign,'--format','text'),text=True)
    for label in ['105 charged, 0 held','Protected Final selection:','Independent protected Final measurement:','Model selected evaluation','Full-history source evidence']:
        assert label in text,(label,text)
    assert text.count('Proposal attempt ')==3 and text.count('Development improvement:')==2,text
    # A Final measurement cannot be laundered through the old selected-pair bridge.
    invoke(['strategy','eligibility','prepare','--registry',registry,'--campaign',campaign],success=False)
    evidence=['strategy','search','eligibility']
    prepared=invoke(evidence+['prepare','--registry',registry,'--campaign',campaign])
    expected=prepared['evidence_sha256'];descriptor=prepared['descriptor']
    assert descriptor['schema_version']==2 and descriptor['config_sha256']==report['config_sha256'],descriptor
    import_args=evidence+['import','--registry',registry,'--campaign',campaign,'--command-id','full-search-import','--expected-evidence',expected]
    imported=invoke(import_args);receipt=imported['receipt']
    assert imported['duplicate'] is False and receipt['candidate_generation']==selected['candidate_generation'],imported
    assert 'measured' in receipt['qualification'],receipt
    assert invoke(['strategy','registry','status','--registry',registry])['current']==initial
    with sqlite3.connect(registry) as db:
        assert db.execute('SELECT count(*) FROM strategy_imports').fetchone()==(1,)
        assert db.execute('SELECT count(*) FROM activations').fetchone()==(1,)
    # All earlier work is retained in the portable evidence, including the losing candidate.
    # The receipt reader must reassess it offline, with neither source DB nor model configuration.
    server.shutdown();server.server_close()
    os.unlink(providers);os.unlink(host_file);env.pop('SEARCH_LOCAL_KEY')
    assert invoke(search('run',campaign))['report']==report
    assert invoke(search('report',campaign))['report']==report
    retried=invoke(['strategy','search','create','--command-id','normal','--plan',plan_file])
    ...
    shown=invoke(evidence+['show','--registry',registry,'--receipt',imported['receipt_sha256']])
    assert shown['receipt']==receipt and shown['receipt_sha256']==imported['receipt_sha256'],shown
    invoke(['strategy','eligibility','show','--registry',registry,'--receipt',imported['receipt_sha256']],success=False)
    mismatch=import_args.copy();mismatch[-1]='sha256:'+'0'*64
    invoke(mismatch,success=False)
    assert not os.path.exists(state) and len(models)==35
    assert invoke(['strategy','registry','status','--registry',registry])['current']==initial
