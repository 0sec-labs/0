...
\title{An MCP Client for 0: Attaching External Tool Servers\\
Without Bloating the Tool Surface or the Trust Boundary}

\author{
  0 Labs \\
  \texttt{github.com/0sec-labs/0}
}
...
--- over a small JSON-RPC protocol. 0 has no MCP client today, and a naive one
would fight two properties 0 has deliberately built. First, MCP tool schemas
...
always-loaded budget 0 has just adopted. Second, and more seriously, an
...
designs an MCP-client layer built entirely from machinery 0 already owns: an
...
externally-hosted tool, and 0 already has a security model for that. This is a
...
0 is not an MCP host. It ships a fixed registry of model-facing tools
...
one without regressing the two things 0 has spent effort getting right: a
...
\begin{itemize}[leftmargin=*]
  ...
  and cost \cite{mcp-clientbp,anthropic-toolsearch}. 0's companion
  ...
  \cite{mcp-security}. 0 already treats live-target egress as a guarded
...
(templates); for a first 0 integration only \emph{tools} matter, so the rest
...
Both of the agents 0 most resembles already ship MCP clients, and their
...
Three lessons carry directly into the 0 design:
\begin{itemize}[leftmargin=*]
...
This is the same conclusion 0's tool-curation work reached from the other
...
\section{0 today: the parts we build on}

The integration is far cheaper than it looks because 0 already contains a
...
uses it to expose 0's own tools \emph{as} an MCP server (behind an
...
\paragraph{The capability model.} 0 already admits externally-authored tools
...
\texttt{ZERO\_REQUIRE\_SCOPE} makes every mode fail closed;
...
patterns, wrapping output in \texttt{[[ZERO\_UNTRUSTED\_DATA]]} with a
...
per-user global (\texttt{\textasciitilde/.0/tui-settings.json}) with a
per-project override (\texttt{<cwd>/.0/tui-settings.json}), described by
\texttt{SETTING\_DEFS} in \texttt{packages/cli/src/tui/settings.ts} and operated
by \texttt{0 config show/export/import}
...
(\texttt{ZERO\_FEATURE\_*}), most defaulting off.

\section{Design}

The design principle in one line: \textbf{an MCP tool is a model-facing,
externally-hosted tool, and 0 already has a security model for
...
but living inside 0's existing layered config so global/project precedence and
...
attach a server; and a new master flag \texttt{ZERO\_FEATURE\_MCP\_CLIENT}
...
but spawn them with the same hardened child environment 0 already builds for
...
\cite{claudecode-mcp} --- so MCP tools never collide with 0 built-ins and are
...
\begin{enumerate}[leftmargin=*]
  ...
  no 0 capabilities, so the client must \emph{assign} them, and to avoid the
...
Two 0 chokepoints do the heavy lifting:
...
\begin{itemize}[leftmargin=*]
  ...
  closed under \texttt{ZERO\_REQUIRE\_SCOPE} semantics.
\end{itemize}

\subsection{The untrusted-input chokepoint}

MCP tool \emph{results} are untrusted input: they are attacker-influenceable text
returned by a third party and fed back into the model. Here 0 has already done
...
fencing the payload in \texttt{[[ZERO\_UNTRUSTED\_DATA]]} with a
...
threats the MCP security spec enumerates \cite{mcp-security}. For each, 0's
...
\texttt{[[ZERO\_UNTRUSTED\_DATA]]} --- plus truncation and the structural denial
...
\texttt{0 mcp auth <server>} step, following OpenCode \cite{opencode-mcp}),
...
\texttt{ZERO\_FEATURE\_MCP\_CLIENT}; and stdio servers run under the same hardened
...
capability, and a tool the model deliberately loads and calls will run. 0's
...
We will:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Add an opt-in MCP client} behind \texttt{ZERO\_FEATURE\_MCP\_CLIENT}
...
scope stack enforces. Every requirement above is satisfied by machinery 0
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Phase 4 --- remote auth.} Add \texttt{0 mcp auth <server>} for
  ...
  as a catalog-refresh (cache-friendly), add \texttt{0 mcp list/get/remove}
...
0 Labs. \textit{Tool Curation for Security Agents}.
