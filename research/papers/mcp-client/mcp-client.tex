...
\title{An MCP Client for 0sec: Attaching External Tool Servers\\
...
--- over a small JSON-RPC protocol. 0sec has no MCP client today, and a naive one
would fight two properties 0sec has deliberately built. First, MCP tool schemas
...
always-loaded budget 0sec has just adopted. Second, and more seriously, an
...
designs an MCP-client layer built entirely from machinery 0sec already owns: an
...
externally-hosted tool, and 0sec already has a security model for that. This is a
...
0sec is not an MCP host. It ships a fixed registry of model-facing tools
...
one without regressing the two things 0sec has spent effort getting right: a
...
\begin{itemize}[leftmargin=*]
  ...
  and cost \cite{mcp-clientbp,anthropic-toolsearch}. 0sec's companion
  ...
  \cite{mcp-security}. 0sec already treats live-target egress as a guarded
...
(templates); for a first 0sec integration only \emph{tools} matter, so the rest
...
Both of the agents 0sec most resembles already ship MCP clients, and their
...
Three lessons carry directly into the 0sec design:
\begin{itemize}[leftmargin=*]
...
This is the same conclusion 0sec's tool-curation work reached from the other
...
\section{0sec today: the parts we build on}

The integration is far cheaper than it looks because 0sec already contains a
...
uses it to expose 0sec's own tools \emph{as} an MCP server (behind an
...
\paragraph{The capability model.} 0sec already admits externally-authored tools
...
per-user global (\texttt{\textasciitilde/.0sec/tui-settings.json}) with a
per-project override (\texttt{<cwd>/.0sec/tui-settings.json}), described by
\texttt{SETTING\_DEFS} in \texttt{packages/cli/src/tui/settings.ts} and operated
by \texttt{0sec config show/export/import}
...
externally-hosted tool, and 0sec already has a security model for
...
but living inside 0sec's existing layered config so global/project precedence and
...
but spawn them with the same hardened child environment 0sec already builds for
...
\cite{claudecode-mcp} --- so MCP tools never collide with 0sec built-ins and are
...
\begin{enumerate}[leftmargin=*]
  ...
  no 0sec capabilities, so the client must \emph{assign} them, and to avoid the
...
Two 0sec chokepoints do the heavy lifting:
...
returned by a third party and fed back into the model. Here 0sec has already done
...
threats the MCP security spec enumerates \cite{mcp-security}. For each, 0sec's
...
\texttt{0sec mcp auth <server>} step, following OpenCode \cite{opencode-mcp}),
...
capability, and a tool the model deliberately loads and calls will run. 0sec's
...
scope stack enforces. Every requirement above is satisfied by machinery 0sec
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Phase 4 --- remote auth.} Add \texttt{0sec mcp auth <server>} for
  ...
  as a catalog-refresh (cache-friendly), add \texttt{0sec mcp list/get/remove}
...
0sec Labs. \textit{Tool Curation for Security Agents}.
