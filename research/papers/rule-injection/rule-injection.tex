% 0sec: Just-in-Time Rule Injection design note
...
document the design decision for a rule-injection layer in 0sec that sits on
top of its existing dynamic playbooks and just-in-time (JIT) skills. The 0sec
...
0sec today injects guidance through two dynamic mechanisms and one static one,
...
(ii) identify the specific gap in 0sec's current stack, and (iii) fix the schema,
...
\textbf{0sec rules (proposed)} & regex on tool input & post-dispatch turn & action \\
\bottomrule
\end{tabular}
...
match against the live output stream and injecting mid-generation. 0sec's
...
0sec's design takes the same principle to the finest grain: surface one line only
at the moment its specific action is imminent.

\section{0sec Today, and the Gap}
\label{sec:gap}

0sec has two JIT-ish dynamic layers. Both trigger identically: a
...
\section{The 0sec Design}
\label{sec:design}

We adopt the TTSR insight --- correct the action, not the aftermath --- but
adapt it to 0sec's execution model. 0sec's agent loop is turn-based over a native
...
playbook injection. This is deliberately \emph{not} a mid-token abort: 0sec's
...
We will add a rule-injection layer to 0sec as specified in
...
cost is not justified by 0sec's turn-based loop at v1.
...
rewrite on a match --- would close this window and bring 0sec closer to TTSR's
