...
\begin{abstract}
0 now has two ways to run more than one agent, and they are deliberately
...
0 now has \textbf{two} ways to run more than one agent, and they are
...
all drawn from \textbf{Oh My Pi} \citep{omp}. Where 0 diverges --- the pure,
injected park loop; the Zod-validated spawn tool; the single-chokepoint inbound
sanitiser; the authority-free addressing policy --- it is to fit 0's security
