...
|---|---|---|
| `self-scan` | TS/JS in the language families 0 ships in. Catches regressions from our own dogfood. | 0 repo itself. |
...
0#254 as **evaluated, not defaulted**, and document the failure
...
**Follow-up issue (if applicable):** 0#<number>
```
...
node packages/benchmark/scripts/foxguard-ablation.mjs \
  ...
  --npm-bench-cache /tmp/0-npm-bench-cache
```
...
[`packages/core/src/shared-analysis.ts`](https://github.com/0sec-labs/0/blob/main/packages/core/src/shared-analysis.ts)
...
- 0#254 — sibling runner + ablation (this work)
- 0#116 — prior ablation pattern (egats removal) — informs how
