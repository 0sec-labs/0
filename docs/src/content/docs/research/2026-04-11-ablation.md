...
We filed [0#116](https://github.com/0sec-labs/0/issues/116) to disable EGATS in default aliases. The code remains available but opt-in. A separate postmortem is planned to document where the implementation diverges from MAPTA-style scoring.
...
**Shipped today:** per-finding layer verdicts telemetry (commit [`6f1a889`](https://github.com/0sec-labs/0/commit/6f1a889), closes [0#112](https://github.com/0sec-labs/0/issues/112)). Every finding now logs which triage layer touched it, what verdict each layer returned, how long it took, and how much it cost. This is the training signal we need for the next thing.

**Shipped today:** egats excluded from the `moat` and `moat-only` profile aliases in CI ([0#116](https://github.com/0sec-labs/0/issues/116)). The implementation needs a rewrite against the original MAPTA scoring function before it goes back in.
...
- [0#72](https://github.com/0sec-labs/0/issues/72) — the ablation matrix issue, with run IDs and per-comment result tables
- [0#111](https://github.com/0sec-labs/0/issues/111) — the npm-bench "recall problem" that turned out not to exist on the live test set (closed)
- [0#112](https://github.com/0sec-labs/0/issues/112) — per-finding layer verdicts telemetry (closed by commit `6f1a889`)
- [0#113](https://github.com/0sec-labs/0/issues/113) — learned dynamic routing for triage (open)
- [0#114](https://github.com/0sec-labs/0/issues/114) — `triage-dataset-v1.jsonl` generation (closed by commit `f40e1c1`)
- [0#116](https://github.com/0sec-labs/0/issues/116) — disable egats in default profile (closed by commit `aadcf32`)
...
- The follow-on routing work is tracked in [Dynamic Routing Design](/research/dynamic-routing-design/) and [0#113](https://github.com/0sec-labs/0/issues/113).
