...
description: "Measured results from a 21-profile ablation of 0's triage stack across XBOW white-box, XBOW black-box, npm-bench, and stubborn-14."
---
...
The study exercised 0's target discovery, shell probes, and structured findings.
...
We filed [0sec#116](https://github.com/0sec-labs/0/issues/116) to disable EGATS in default aliases. The code remains available but opt-in. A separate postmortem is planned to document where the implementation diverges from MAPTA-style scoring.
...
- **BoxPwnr** ([0ca](https://github.com/0ca)) — context compaction, loop detection, and progress handoff patterns informed corresponding 0 features.
...
- [0sec#72](https://github.com/0sec-labs/0/issues/72) — the ablation matrix issue, with run IDs and per-comment result tables
- [0sec#111](https://github.com/0sec-labs/0/issues/111) — the npm-bench "recall problem" that turned out not to exist on the live test set (closed)
- [0sec#112](https://github.com/0sec-labs/0/issues/112) — per-finding layer verdicts telemetry (closed by commit `6f1a889`)
- [0sec#113](https://github.com/0sec-labs/0/issues/113) — learned dynamic routing for triage (open)
- [0sec#114](https://github.com/0sec-labs/0/issues/114) — `triage-dataset-v1.jsonl` generation (closed by commit `f40e1c1`)
- [0sec#116](https://github.com/0sec-labs/0/issues/116) — disable egats in default profile (closed by commit `aadcf32`)
- [triage-dataset-v1.jsonl](https://github.com/0sec-labs/0/blob/main/packages/benchmark/results/triage-dataset-v1.jsonl) — 969 labeled rows, the first training-data artifact
...
- The follow-on routing work is tracked in [Dynamic Routing Design](/research/dynamic-routing-design/) and [0sec#113](https://github.com/0sec-labs/0/issues/113).
