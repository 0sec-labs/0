...
| **0** | **93/95 = 97.9%** black-box | Azure gpt-5.4 | Shell-first, single-model single-shot cohort | ~$0.48/run, **$5.20/flag** |

Two distinctions:

- **Best-of-N ≠ single-config.** BoxPwnr's 97.1% is a union over ~10 model+solver
  configs (~5 attempts/challenge); its best *single* model scores 81.7%. 0's
