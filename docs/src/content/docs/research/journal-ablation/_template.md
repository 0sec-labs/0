---
draft: true
title: "Journal-Loop A/B Ablation ({{DATE}})"
description: "Measured A/B results for the journal-driven orchestrator loop vs. the existing native loop, across XBOW black-box, XBOW white-box, and npm-bench. Drives the promotion decision for ZERO_FEATURE_JOURNAL_LOOP."
---

*Auto-generated by `.github/workflows/journal-ablation.yml` (run `{{RUN_ID}}`, commit `{{COMMIT_SHA}}`).*

> **Archived template, not an executed result.** The referenced workflow and
> aggregator are absent from this checkout. `{{...}}` fields and promotion
> thresholds below are the historical protocol, not a working CI trigger or
> evidence that a journal-loop comparison has run.

## Methodology

- Both arms run on the same git ref, same model, same prompts. The only
  difference is `ZERO_FEATURE_JOURNAL_LOOP` (0 for native, 1 for journal).
- Three slices: XBOW black-box, XBOW white-box, npm-bench. Each uses three repeats
  per arm, with Wilson 95% intervals for found-rate proportions and
  normal-approximation 95% intervals for continuous metrics.
- Results aggregate raw per-run JSONs uploaded as workflow artifacts. The
  aggregator lives at `.github/workflows/scripts/journal-ab-aggregate.mjs`.

## Promotion gate

The flag flips to default ON only when ALL of the following hold:

- **XBOW BB:** journal found-rate `journal_mean` is **>= 97/104** on the
  patched-104 lookback window. No statistical regression vs. native is
  acceptable here — this is the state-of-record slice.
- **Any slice:** cost regression `delta_pct` on the `cost ($)` row is
  **<= +10%** (i.e. journal is at most 10% more expensive per run).
- **XBOW WB:** wall-clock `delta_pct` on the `wall (s)` row is
  **<= +50%** (i.e. journal is at most 1.5x the native wall-clock).

If any of those checks fails, the flag stays OFF and the design doc records
the gap.

> **Status note (audited 2026-08-22):** the `journalLoop` flag and the
> single-point dispatcher described here were never built — there is no
> `loop-dispatch.ts`, and `features.ts` has no `journalLoop` entry.
> `packages/core/src/agent/native-loop.ts` remains the only agent loop;
> `packages/core/src/agent/journal/` holds the journal primitives. This
> ablation protocol is retained as the plan of record, not as shipped wiring.

## Results

{{TABLE}}

## Decision

<pending — fill in after reviewing the table above>

- [ ] XBOW BB regression check
- [ ] Cost regression check (<= 10% on all slices)
- [ ] Wall-clock check on XBOW WB (<= 1.5x native)
- [ ] Decision: flip default | keep OFF | rerun

Recorded by: <pending>
Decision date: <pending>
