...
`egats` has been flagged for disable-by-default in [0#116](https://github.com/0sec-labs/0/issues/116).

<span id="takeaways"></span>

1. No single static policy wins on all three slices. The moat helps on black-box XBOW, costs 2 flags on white-box XBOW, and is a batch-1 no-op on npm-bench. Direct motivation for learned dynamic routing — see [0#113](https://github.com/0sec-labs/0/issues/113).
...
5. Per-layer telemetry (`layerVerdicts`) is live on findings after 2026-04-11 — supervision signal for learned routing ([0#113](https://github.com/0sec-labs/0/issues/113)). See [0#112](https://github.com/0sec-labs/0/issues/112) for the instrumentation commit.
...
<span id="foxguard--0-cross-validation"></span>
