% 0sec: arXiv submission draft
% Compile: pdflatex 0sec-submission.tex && pdflatex 0sec-submission.tex
...
\title{0sec: Methodology-Aware Evaluation and Cost-Aware Triage for Autonomous Pentesting Agents}
...
present 0sec, an open-source shell-first pentesting framework with layered
...
publication lines. In the current public ledger snapshot (2026-04-10), 0sec
...
For web targets, 0sec uses shell-first execution (\texttt{bash} as primary
...
Current 0sec docs position this work adjacent to three threads:
...
Representative systems in the current 0sec docs include BoxPwnr, Shannon,
KinoSec, Cyber-AutoAgent, deadend-cli, and
MAPTA\citep{boxpwnr,shannon,kinosec,cyberautoagent,deadend,mapta}.
For FP-triage architecture context, 0sec docs reference Endor Labs and
Semgrep Assistant disclosures\citep{endor,semgrep}. For hybrid feature+model
signals, 0sec docs use VulnBERT-style framing as design inspiration rather
...
\textbf{0sec (this work)}         & \textbf{95.2\% (99/104)} & Shell-first, gpt-5.4   & Retained artifact-backed; see Table~\ref{tab:ledger} \\
...
core contribution. 0sec's current evidence suggests that transparent protocol
