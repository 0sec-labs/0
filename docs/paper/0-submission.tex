% 0: arXiv submission draft
% Compile: pdflatex 0-submission.tex && pdflatex 0-submission.tex
...
\title{0: Methodology-Aware Evaluation and Cost-Aware Triage for Autonomous Pentesting Agents}
...
present 0, an open-source shell-first pentesting framework with layered
...
publication lines. In the current public ledger snapshot (2026-04-10), 0
...
For web targets, 0 uses shell-first execution (\texttt{bash} as primary
...
Current 0 docs position this work adjacent to three threads:
...
Representative systems in the current 0 docs include BoxPwnr, Shannon,
KinoSec, Cyber-AutoAgent, deadend-cli, and
MAPTA\citep{boxpwnr,shannon,kinosec,cyberautoagent,deadend,mapta}.
For FP-triage architecture context, 0 docs reference Endor Labs and
Semgrep Assistant disclosures\citep{endor,semgrep}. For hybrid feature+model
signals, 0 docs use VulnBERT-style framing as design inspiration rather
...
\textbf{0 (this work)}         & \textbf{95.2\% (99/104)} & Shell-first, gpt-5.4   & Retained artifact-backed; see Table~\ref{tab:ledger} \\
...
core contribution. 0's current evidence suggests that transparent protocol
