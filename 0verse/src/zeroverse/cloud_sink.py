"""Engine-side cloud sink — stream 0verse findings to the 0cloud orchestrator.
...
class CloudSinkConfig:
    ...
    events: bool = True  # ZERO_CLOUD_EVENTS — stream per-finding POSTs live
...
def build_config(
    *, sink: str | None = None, scan_id: str | None = None, timeout_ms: int = 30_000
    ...
    error in ``--cloud`` mode).
    """
    sink_url = (os.environ.get("ZERO_CLOUD_SINK") or sink or "").strip()
    sid = (os.environ.get("ZERO_CLOUD_SCAN_ID") or scan_id or "").strip()
    if not sink_url or not sid:
        return None
    token = (os.environ.get("ZERO_CLOUD_TOKEN") or "").strip() or None
    org_id = (os.environ.get("ZERO_CLOUD_ORG_ID") or "").strip() or None
    events = os.environ.get("ZERO_CLOUD_EVENTS", "1").strip() != "0"
