...
def test_build_config_env_precedence_and_token_from_env(monkeypatch: Any) -> None:
    monkeypatch.setenv("ZERO_CLOUD_SINK", "https://env.example/api")
    monkeypatch.setenv("ZERO_CLOUD_SCAN_ID", "env-scan")
    monkeypatch.setenv("ZERO_CLOUD_TOKEN", "env-token")
    monkeypatch.setenv("ZERO_CLOUD_ORG_ID", "org-7")
...
def test_events_flag_disables_per_finding_posts(monkeypatch: Any) -> None:
    monkeypatch.setenv("ZERO_CLOUD_EVENTS", "0")
...
def test_cli_cloud_invokes_run_cloud_scan(monkeypatch: Any) -> None:
    ...
    monkeypatch.setenv("ZERO_CLOUD_TOKEN", "env-only-token")
    monkeypatch.delenv("ZERO_CLOUD_SINK", raising=False)
    monkeypatch.delenv("ZERO_CLOUD_SCAN_ID", raising=False)
